...
fn get_mem_gpu () {
	...
	unsafe {
		...
		debug!("sys::vcos_safe_strcpy(buffer_ptr, b\"get_mem\\0\".as_ptr(), {}, {});", buffer.len(), offset);
		offset = sys::vcos_safe_strcpy(buffer_ptr, b" \0".as_ptr()      , buffer.len(), offset);
		debug!("sys::vcos_safe_strcpy(buffer_ptr, b\" \\0\".as_ptr()      , buffer.len(), {});", offset);
		offset = sys::vcos_safe_strcpy(buffer_ptr, b"gpu\0".as_ptr()    , buffer.len(), offset);
		debug!("sys::vcos_safe_strcpy(buffer_ptr, b\"gpu\\0\".as_ptr()    , buffer.len(), {});", offset);
